            ...
            >
                ...
                >
                    <SortableContext items={lists.map(l => formatListId(l.id))} strategy={horizontalListSortingStrategy}>
                        <div
                            className={`flex gap-4 sm:gap-8 items-start min-h-full mb-20 snap-x snap-mandatory transition-transform duration-300 origin-top-left ${isZoomedOut ? 'scale-70 w-[142%]' : 'scale-100 w-full'}`}
                        >
                            {lists.map((list) => (
                                <List
                                    key={list.id}
